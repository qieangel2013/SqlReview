...
func InitMySQLPool(mysql_conf MysqlConfType) *SQLConnPool {
	dataSourceName := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=%s&autocommit=true", mysql_conf.MysqlUser, mysql_conf.MysqlPw, mysql_conf.MysqlHost, mysql_conf.MysqlPort, mysql_conf.MysqlDb, mysql_conf.MysqlCharset)
	db := &SQLConnPool{
		DriverName:     "mysql",
		DataSourceName: dataSourceName,
		MaxOpenConns:   mysql_conf.MysqlMaxOpenConns,
		MaxIdleConns:   mysql_conf.MysqlMaxIdleConns,
	}
	if err := db.Open(); err != nil {
		Error.Println("Init mysql pool failed.", err.Error())
	}
	return db
}
