...
// 设置请求Host
func (this *Request) SetHost(host string) *Request {
	this.Host = host
	return this
}
...
// 设置post请求的提交数据
func (this *Request) SetPostData(postData ExplainsResult) *Request {
...
func (this *Request) Send(url string, method string) (*Response, error) {
	...
	if method == "POST" {
		if jData, err := JsonSqlExpEncode(this.PostData); err != nil {
			return nil, err
		} else {
